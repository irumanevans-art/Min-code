...
public final class PairingConnectionCtx implements Closeable {
    ...
    // min-code 改动：原版用反射调 Conscrypt 的 exportKeyingMaterial。
    // Android 12 (API 31) 起有等价的公开 API android.net.ssl.SSLSockets.exportKeyingMaterial，
    // 签名与语义一致，直接用它，去掉 Conscrypt 依赖。应用内配对已门控到 API 31+。
    @RequiresApi(Build.VERSION_CODES.S)
    private byte[] exportKeyingMaterial(SSLSocket sslSocket, int length) throws SSLException {
        return android.net.ssl.SSLSockets.exportKeyingMaterial(sslSocket, EXPORTED_KEY_LABEL, null, length);
    }
