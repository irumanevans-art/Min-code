...
class ClaudeCodeStderrTest {
    ...
    @Test
    fun `第一行新建一条`() {
        val items = appendStderrLine(emptyList(), "npm warn deprecated", "a")
    ...
    fun `连续的行并进同一条`() {
        ...
        items = appendStderrLine(items, "一", "a")
        items = appendStderrLine(items, "二", "b")
        items = appendStderrLine(items, "三", "c")
        assertEquals(1, items.size)
    ...
    @Test
    fun `中间夹了别的条目就另起一条`() {
        var items: List<ChatItem> = appendStderrLine(emptyList(), "一", "a")
    ...
    fun `超出上限丢最旧并记下丢了多少`() {
        var items: List<ChatItem> = emptyList()
        repeat(STDERR_MAX_LINES + 5) { i ->
            items = appendStderrLine(items, "第 $i 行", "id$i")
        }
        val block = items.single() as ChatItem.ProcessOutput
        assertEquals(STDERR_MAX_LINES, block.lines.size)
        assertEquals(5, block.dropped)
        // 留下的是最新的那一批
        assertEquals("第 ${STDERR_MAX_LINES + 4} 行", block.lines.last())
        assertEquals("第 5 行", block.lines.first())
    }

    @Test
    fun `单行过长时截断`() {
        val items = appendStderrLine(emptyList(), "x".repeat(STDERR_LINE_CHARS * 3), "a")
        assertEquals(STDERR_LINE_CHARS, lines(items).single().length)
    }
    ...
    fun `噪声行也留下但不算错误`() {
        ...
        val items = appendStderrLine(emptyList(), noise, "a")
