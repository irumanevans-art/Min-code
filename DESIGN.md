...
**形本身就是风格语义**的地方：浪会卷，云不会卷，陶 / Anthropic 的收笔是线长出来的一小段结构。三套共用一个海浪卷的话，
...
- 会话流是纸；左侧轨道是一道 1.5 dp 的强调色（`TranscriptRail.kt`）。标记：点 = 你，环 = 思考，实心方 = 正在跑（带涟漪），
  实心方 + 定环 = 等你批准，空心方 = 已完成，叉 = 出错（朱），短横 = 提示。空心符号后面垫纸，线不从符号中间穿过。**标记是语义不是风格，三套一样。**
  「等你批准」和「正在跑」共用实心方，差别只在**动与静**：涟漪停在行程七成处不再扩散，整段轨道的流动也一并停掉，
  折叠行右侧的转圈换成一句「等你批准」。手机上人常常切出去再回来，一屏卡片全在转时分不清哪张在等自己——
  静止本身就是「卡在这里了」。**不换色**：朱留给判定的结果（拒绝、中断），等待本身还是海。
  **线身、收笔、活段上那点动则按风格三选一**（`RailStyle.kt`，全 App 唯一一处换形的地方，理由见下「三张取底」）：
  ...
  | 云 `CloudRail` | 一段段断开的雾（6~22 dp），两端渐隐、中部最实；**一带浓一带淡**（每 3 段换一次气）——少了这层，长短随机的段会平均成一条等距虚线 | 最后一段雾散成三缕雾丝：都从轴线上出发，相位错开地往下、微微往右摆，途中彼此交缠，越远越细越淡，到底散尽（灰白仙境顶上那道云涡的局部）。丝外两层软晕，没有硬边；夜纸上丝心再提一线浪沫 | 一道 40 dp 宽的透光自上而下掠过，窗扫到哪里哪里的雾就亮起来并叠一线浪沫；雾丝自己还在慢慢飘。**没有粒子**（3600 ms） |
  | 陶 `ClayRail` | 手拉的颤线：两频叠加抖 ±0.72 dp、笔宽 0.9~1.7 dp 不匀，每 37 dp 掷一次「这一刀有没有提笔」（飞白） | 竖线收细落到一颗主节点，长出三根短杆、杆头各结一颗更小的节点，右上那根再分一级——线自己长成一小段分子结构（Claude 登录页那幅画的构成方式），**不是**挂在线下面的一枚徽章；杆接着线的粗细 | 一滴釉缓缓下淌（4200 ms，比白沫慢、尾更长更柔），淌进主节点；节点按相位依次亮起，像结构在长 |
...
- 输入坞（`ComposerDock`）：浮在会话流上，**没有分界线**，胶囊本身就是边界。输入框是一只 24 dp 圆角的胶囊
  ...
  **胶囊以下不再有任何一行**：那条安静的状态（模型摘要 · 上下文 · 花费）整条收进了「+」——它是常驻的参考信息，
  ...
  Codex 会话页用的是同一只胶囊（`ComposerCapsule`，外观只写在这一处），同样的纸色渐隐与磨砂；差别只在「+」里的内容：
  没有用量那一栏（Codex 回报的是累计用量，不是上下文占用，也没有花费）、没有拍照 / 相册（localImage 还没接），
  设置栏是模型 · 权限 · 强度。排队中的追加消息是 Codex 独有的，摆在胶囊正上方。
- 会话设置（`ClaudeCodeSettingsSheet`）：`InkSheet` + 一粒 `Seal` 的标题，正在应用时压一条忙碌横幅。
  ...
  Codex 的会话设置（`CodexSettingsSheet`）是同一张面板——标题、横幅、手风琴、选中行、保活那一行只写在
  `SessionSettingsParts.kt`，两边共用。分区换成 Codex 自己有的：模型 / 权限 / 思考强度 / 思考摘要 / 工作目录；
  没有提示缓存和斜杠命令表（app-server 协议里没有）。权限照 codex CLI 的 `/permissions` 给三档（只读 / 自动 / 完全访问），
  每档底下一行等宽小字写实际发出去的 `approvalPolicy · sandboxPolicy`——档名是给人读的，那两个值是给对照文档和报错用的。
  这些值都是 `turn/start` 的逐轮覆盖，点了就记下、下一轮带上，所以不变灰、不等；一轮正跑着时横幅只说「从下一轮起生效」。
...
- 长期运动：发送键里的海、首页字标上走的光与折射、轨道上那点动（海的白沫 / 云的透光与雾丝 / 陶的釉与节点，见上表）、涟漪、螺旋的呼吸、岸边的浪沫。海本身不动。系统动画关闭时全部静止，显示完整静态内容。
