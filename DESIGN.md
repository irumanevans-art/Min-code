...
- 会话流铺满整屏。顶栏和输入坞浮在上面，只留主要元素（菜单、Min、停止；输入胶囊）。
  铬件自己的位置是纸色渐隐（`frostVeil` / `Frost.kt`）：对齐 ChatGPT —— **不是高斯糊**。从左上圆钮下沿 / 输入胶囊上沿起，越靠近屏沿，会话和花纹越被纸色盖住，接到底色；切线处纸色是 0，软边只有 [FrostFade]（8 dp）。糊只留给圆钮自己（`frostPane`）。顶栏 Min 底下垫一圈实的白（浅色纸上虚的高斯白等于没有），涨出布局、父级不裁。输入胶囊以下只剩系统导航让出的位置，整段留空收成一半。
...
- 输入坞（`ComposerDock`）：浮在会话流上，**没有分界线**，胶囊本身就是边界。输入框是一只 24 dp 圆角的胶囊
  （最亮的纸 + hairline，聚焦时边变成海的深处）：左端一个「+」（用量、会话设置与文件 / 图片全收进这一个小菜单），右端发送键——只有一个朝上的「>」
  ...
  **胶囊以下不再有任何一行**：那条安静的状态（模型摘要 · 上下文 · 花费）整条收进了「+」——它是常驻的参考信息，
  ...
  忙时那枚朱色中断不进菜单，留在胶囊里、发送键左边——要停的时候不该先点开一个菜单。正在跑的精确阶段（思考 / 执行工具 / 压缩）挂在会话流末尾，不在输入框上面占一行「请求中 / 停止」。
