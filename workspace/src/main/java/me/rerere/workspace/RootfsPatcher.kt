...
class RootfsPatcher {
    fun patch(
        ...
        // 会话 MAX_CONCURRENT=3，并发启动时几个 patcher 同时改同一棵 rootfs 是常态：
        // resolv.conf 的 delete→write、hosts/group 的 read→append 都是 read-modify-write，
        // 不串行化就会交错出半截文件。按 linuxDir 加进程内锁（patcher 是临时 new 出来的，
        // 锁必须放 companion）。
        val lock = PATCH_LOCKS.getOrPut(linuxDir.canonicalPath) { ReentrantLock() }
        lock.withLock {
    ...
    private fun ensureRootfsDns(
        ...
        // 符号链接必须先删：改名 / 拷贝都会作用到链接**目标**上，会写到 rootfs 外面
        ...
        // 写临时文件再改名：读者（guest 进程）不会看到写了一半的 resolv.conf
        writeAtomic(
            resolvConf,
            buildString {
                appendLine("# Generated by Min workspace.")
    ...
    /** 写临时文件再 rename；目标已存在且 renameTo 失败时（Windows）退回拷贝覆盖 */
    private fun writeAtomic(file: File, content: String) {
        val tmp = File(file.parentFile, file.name + ".min-tmp")
    ...
    private companion object {
        ...
        /** 按 linuxDir canonicalPath 的进程内锁：patcher 是用即抛的实例，锁必须共享 */
        private val PATCH_LOCKS = ConcurrentHashMap<String, ReentrantLock>()
