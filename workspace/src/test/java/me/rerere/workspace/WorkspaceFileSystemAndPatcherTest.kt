...
class WorkspaceFileSystemAndPatcherTest {
    @Test
    ...
    fun mkdirCreatesADirectoryInsideTheWorkspace() {
        ...
        val fileSystem = WorkspaceFileSystem()
        val entry = fileSystem.mkdir(root, "src/app")
        assertTrue(entry.isDirectory)
        assertEquals("src/app", entry.path)
        assertEquals(listOf("src"), fileSystem.list(root).map { it.path })
        assertEquals(listOf("src/app"), fileSystem.list(root, "src").map { it.path })
    }

    @Test
    fun renameAndMoveStayInsideTheWorkspace() {
        val root = Files.createTempDirectory("workspace-move").toFile()
        val fileSystem = WorkspaceFileSystem()
        fileSystem.mkdir(root, "src")
        fileSystem.mkdir(root, "dst")
        fileSystem.writeText(root, "src/notes.md", "hello")

        fileSystem.move(root, "src/notes.md", "src/readme.md")
        assertEquals("hello", fileSystem.readText(root, "src/readme.md"))

        fileSystem.move(root, "src/readme.md", "dst/readme.md")
        assertEquals("hello", fileSystem.readText(root, "dst/readme.md"))
        assertEquals(listOf("dst/readme.md"), fileSystem.list(root, "dst").map { it.path })
    }
